...
{
    protected ?BigQueryClient $bigQuery = null;
    protected ?Dataset $dataset = null;
    protected ?Table $table = null;

    // Stored config for lazy initialization
    ...
    {
        // Only store config — BigQueryClient is initialized lazily on first use
        $this->projectId = config('services.bigquery.project_id');
        $this->keyFilePath = config('services.bigquery.key_file');
     ...
     * This prevents constructor crashes when credentials file is missing.
     */
    private function getBigQuery(): BigQueryClient
    {
        if ($this->bigQuery === null) {
            ...
            $this->table = $this->dataset->table($this->tableName);
        }
    ...
    {
        $bq = $this->getBigQuery();
        $queryConfig = $bq->query($query);
        if (!empty($params)) {
            $queryConfig->parameters($params);
        }
        $queryResults = $bq->runQuery($queryConfig);
    ...
    public function getIndicatorData(array $params, string $datasetId = null, string $tableId = null): array
    {
        ...
        try {
            // Null-safe: agar per_page null aaye (no-pagination mode) to large default use karo
            // (int) null = 0 hota hai jo LIMIT 0 bana deta tha → koi data nahi aata tha
            $perPage = isset($params['per_page']) && $params['per_page'] !== null
                ? max(1, (int) $params['per_page'])
                : 700000; // no-pagination mode: sabhi rows fetch karo
            ...
            $countQuery = "SELECT COUNT(*) as total_count FROM {$fullTablePath} {$whereClause}";

            // Cache key: data query ke liye (page-specific)
            $baseParams = array_diff_key($params, array_flip(['page', 'per_page']));
            $dataCacheKey = 'bq_indicator_data_' . md5(json_encode($baseParams) . $fullTablePath . "_p{$page}_pp{$perPage}");
            // Cache key: count query ke liye (page-independent — sirf filters pe depend karta hai)
            $countCacheKey = 'bq_indicator_count_' . md5(json_encode($baseParams) . $fullTablePath);

            $isFromCache = \Cache::has($dataCacheKey);

            $rows = \Cache::remember($dataCacheKey, now()->addMinutes(15), function () use ($dataQuery, $bindings) {
                return $this->runQuery($dataQuery, $bindings);
            });

            // Total count alag cache me store hoga (sirf ek baar BigQuery se fetch hoga per filter combination)
            $totalRecords = \Cache::remember($countCacheKey, now()->addMinutes(15), function () use ($countQuery, $bindings) {
                $countRows = $this->runQuery($countQuery, $bindings);
                return !empty($countRows) ? (int) $countRows[0]['total_count'] : 0;
            });
            $executionTime = round((microtime(true) - $startTime) * 1000, 2);

            // Detailed Success Log
            // \Log::info('BigQuery: Data successfully fetched.', [
            //     'table'             => $fullTablePath,
            //     'total_found'       => $totalRecords,
            //     'returned_count'    => count($rows),
            //     'source'            => $isFromCache ? 'Laravel_Cache' : 'BigQuery_Live',
            ...
            // Rich Error Log context ke sath taaki debug karna aasan ho
            // \Log::error('BigQuery Data Fetch Error: ' . $e->getMessage(), [
            //     'exception'          => get_class($e),
            //     'resolved_table'     => $fullTablePath ?? 'Not Resolved',
            //     'execution_time_ms'  => $executionTime,
            //     'request_params'     => $params,
            //     'bindings_attempted' => $bindings ?? null,
            //     'generated_query'    => $dataQuery ?? null,
