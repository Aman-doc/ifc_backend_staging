...
class ThemeController extends Controller
{
    ...
    /**
     * Fetch theme indicators data with pagination and flexible filters.
     */

    // public function getThemeWithIndicators(Request $request, BigQueryService $bigQueryService)
    // {
    //     // Telemetry tracking ke liye execution start time
    ...
    //     // 1. Prepare payload using Indicator ID directly 
    //     $queryParams = [
    //         'indicator' => $indicatorId,
    //         'source'    => $request->get('source'),
    //         'state_id'  => $request->get('state_id'),
    //         'year'      => $request->get('year'),
    //         'per_page'  => $request->get('per_page', 2000),
    ...
    //     // 3. Process Rows and Flatten JSON Filters
    //     foreach ($rows as $row) {
    //         $item = [
    ...
    //     // // 4. Final Telemetry Log
    //     \Log::info('Theme Indicators API Successfully Executed.', [
    //         'indicator_id'      => $queryParams['indicator'],
    //         'total_rows'        => $bqResult['total'] ?? 0,
    //         'processed_count'   => count($dataList),
    //         'execution_time_ms' => $executionTime
    //     ]);

    //     return response()->json([
    //         'pagination' => [
    //             'total'        => $bqResult['total'] ?? 0,
    //             'per_page'     => $bqResult['per_page'] ?? 2000,
    //             'current_page' => $bqResult['current_page'] ?? 1,
    //             'last_page'    => $bqResult['last_page'] ?? 1,
    //         ],
    //         'data' => $dataList,
    //     ]);
    // }
