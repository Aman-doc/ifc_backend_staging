...
<div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
    <div class="flex justify-between items-center pb-5 border-b border-gray-100">
        <div>
            <h1 class="text-xl font-bold text-gray-800">Create Source</h1>
            <p class="text-xs text-gray-500 mt-0.5">Select Data Sources and their respective Indicators</p>
        </div>
    ...
    <form action="{{ route('admin.sources.store') }}" method="POST" class="mt-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-5 mb-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Enter source title" 
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Write description here..." 
                          class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">{{ old('description') }}</textarea>
            </div>
        </div>
                                ...
                                @forelse($ds->indicators as $ind)
                                    ...
                                    <label class="flex items-center justify-between cursor-pointer text-xs text-gray-700 hover:bg-gray-50 p-2 border border-gray-100 rounded-lg transition-all">
                                        <span class="flex items-center gap-2">
                                            {{-- NOTE: Key-value mapping array format --}}
